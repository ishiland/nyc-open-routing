...
class RoutingService:
    ...
    def get_driving_route(
        ...
        try:
            ...
            if use_traffic:
                ...
                sql = text(
                    "SELECT * FROM getdrivingroute(:orig_lat, :orig_lon, :dest_lat, :dest_lon)"
                )
                with self.engine.connect() as conn:
                    result = conn.execute(
                        sql,
            ...
            if use_traffic and len(rows) > 0:
                ...
                traffic_factors = [
                    row.traffic_factor for row in rows if hasattr(row, "traffic_factor")
                ]
                if traffic_factors and all(tf == 1.0 for tf in traffic_factors):
                    logger.warning(
                        "Traffic routing requested but no traffic"
                        " data available (all traffic_factor=1.0)."
                        " Run import_traffic_speeds.py to enable"
            ...
            if len(rows) == 0:
                ...
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        " This may be due to disconnected areas,"
                        " grade separation restrictions, or"
                        " invalid coordinates. Try different"
                        " points or travel modes."
                    ),
                )
    ...
    def get_biking_route(self, orig: str, dest: str, avoid_ferries: bool = False) -> RouteResponse:
        ...
        try:
            sql = text(
                "SELECT * FROM getbikingroute"
                "(:orig_lat, :orig_lon, :dest_lat,"
                " :dest_lon, :avoid_ferries)"
            )
            with self.engine.connect() as conn:
                result = conn.execute(
                    sql,
                ...
                rows = result.fetchall()
                logger.info(
                    f"Found {len(rows)} route segments for biking (avoid_ferries={avoid_ferries})"
                )
            ...
            if len(rows) == 0:
                ...
                if avoid_ferries:
                    detail += (
                        " Try disabling 'Avoid ferries' — a ferry"
                        " may be required to connect these areas."
                    )
    ...
    def get_walking_route(self, orig: str, dest: str, avoid_ferries: bool = False) -> RouteResponse:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                rows = result.fetchall()
                logger.info(
                    f"Found {len(rows)} route segments for walking (avoid_ferries={avoid_ferries})"
                )
        ...
        try:
            ...
            for i in range(len(waypoints) - 1):
                ...
                if mode == "drive":
                    ...
                    raise HTTPException(status_code=400, detail=f"Unsupported travel mode: {mode}")
                ...
                leg_distance = sum(f.properties.distance or 0 for f in leg_response.features)
                leg_time = sum(f.properties.travel_time or 0 for f in leg_response.features)
            ...
            raise HTTPException(status_code=500, detail="Error processing waypoint route request.")

        response = WaypointRouteResponse(
            legs=legs,
            summary=WaypointRouteSummary(
                total_distance=total_distance, total_travel_time=total_time, num_legs=len(legs)
            ),
        )
    ...
    def _format_route_response(self, rows, result, mode: str = "drive") -> List[Feature]:
        ...
        for row in rows:
            ...
            if mode in ("bike", "walk"):
                ...
                traffic_factor_value = row_dict.get(
                    "traffic_factor", 1.0
                )  # Default to 1.0 (no traffic) if missing
