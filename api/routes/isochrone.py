...
def get_isochrone(
    ...
    mode: TravelMode = Query(default=TravelMode.DRIVE, description="Travel mode: drive, bike, or walk"),
    intervals: Optional[str] = Query(
        default=None,
        description="Comma-separated time intervals in minutes (e.g., '5,10,15,20'). Default: 5,10,15,20",
    ),
    ...
    hour: Optional[int] = Query(default=None, ge=0, le=23, description="Hour of day for traffic (0-23)"),
    day_of_week: Optional[int] = Query(default=None, ge=1, le=7, description="Day of week for traffic (1=Mon, 7=Sun)"),
    ...
    if intervals:
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid intervals format. Use comma-separated numbers (e.g., '5,10,15,20').")
