                ...
                features=[
                    Feature(
                        properties=Properties(
                            seq=1, street="BROADWAY", distance=500.0, travel_time=10.0
                        ),
                        geometry={
                            "type": "LineString",
                            "coordinates": [[-73.98, 40.75], [-73.99, 40.76]],
                        },
                    )
                ],
            )
...
def test_waypoint_route_endpoint_too_few_waypoints():
    ...
    response = client.get("/api/route/waypoints?waypoints=-73.9857,40.7484&mode=drive")
    assert response.status_code == 400
...
def test_waypoint_route_endpoint_too_many_waypoints():
    ...
    response = client.get(
        "/api/route/waypoints?"
        "waypoints=-73.98,40.75|-73.99,40.76"
        "|-74.00,40.77|-73.97,40.74&mode=drive"
    )
    assert response.status_code == 400
...
def test_waypoint_route_endpoint_invalid_coordinates():
    ...
    response = client.get("/api/route/waypoints?waypoints=invalid|-73.99,40.76&mode=drive")
    assert response.status_code == 400
    assert "Invalid coordinate format" in response.json()["detail"]
...
def test_waypoint_route_endpoint_missing_mode():
    ...
    response = client.get("/api/route/waypoints?waypoints=-73.98,40.75|-73.99,40.76")
    assert response.status_code == 422  # FastAPI validation error
...
def test_traffic_tile_valid_z_boundary():
    ...
    with (
        patch("api.routes.traffic.get_tile_cache") as mock_cache,
        patch("api.routes.traffic.get_db_engine"),
    ):
